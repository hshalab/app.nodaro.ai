...
| `resolve_recast_gate` | Record the user's pick at an interactive gate (cast / scene stills / music) and advance the run — the pick is free, pure state. `finish_auto: true` resolves every remaining gate with the critic's top candidate. `workflows:execute`. |
