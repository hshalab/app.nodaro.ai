...
The QA Check node sends upstream text to an LLM (default Claude Sonnet 4.6) and asks it to score the content from 0.0 to 1.0 against a chosen evaluation type. It returns a numeric score, an `approved` boolean (score ≥ threshold), and a short reason. Use it as an automated quality gate inside workflows — for example, to validate generated scripts or captions before they continue downstream.
...
| Model | Select | Claude Sonnet 4.6 | LLM used for the evaluation |
