...
| Planner model | Select | Claude Opus 5 | The AI model that plans the segment breakdown for multi-segment runs — any model from the [LLM model registry](../../choosing-models.md) |
...
- **Planner model** picks which AI model does that planning. The default (Claude Opus 5) works well for most scripts; you can select any model from the [LLM model registry](../../choosing-models.md) to trade speed against planning quality.
