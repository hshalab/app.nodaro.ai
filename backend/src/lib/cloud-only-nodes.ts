/**
 * Node types that exist only on Cloud (`hasCredits()`), for the backend half
 * of the gate.
 *
 ...
 * asserts both sets are identical, so adding a type on one side and forgetting
 * the other fails CI instead of shipping a node that is offered but can't run.
 *
